...
// - 필드 중 final 키워드가 작성된 필드 값을 초기화하기 위한 
//   생성자를 자동 완성하는 Lombok 제공 어노테이션
//
// + final 필드 초기화를 위한 생성자 매개 변수에 
//	 자동으로 같은 자료형의 Bean 을 의존성 주입함
...
//	}
	...
	// 자바에서 타임리프를 사용할 수 있게 하는 객체
	// html 코드를 Java 로 읽어올 수 있음
	
	
	
	// 이메일 발송
	@Override
	public int sendAuthKey(String htmlName, String email) {
		...
		try {
			...
			switch(htmlName) {
			case "signUp" : 
				emailTitle = "[boardProject] 회원 가입 인증번호 입니다."; break;
			case "findPw" :
				emailTitle = "[boardProject] 비밀번호 찾기 인증번호 입니다."; break;
			}
			...
			// HTML 코드 전체가 하나의 String 으로 변환되어서 반환 된 후
			// 반환된 String 을 메일 내용으로 세팅
