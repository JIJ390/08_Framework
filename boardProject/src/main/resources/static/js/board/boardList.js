...
pageNoList?.forEach( (item, index) => {
  ...
  item.addEventListener("click", e => {
    ...
    switch(item.innerText){
      ...
      case '<'  : 
        location.href = "?cp=" + pagination.prevPage;
        break;

      case '>'  : 
        location.href = "?cp=" + pagination.nextPage;
        break;

      case '>>' : 
        location.href = "?cp=" + pagination.maxPage;
        break;

      default: 
        location.href = "?cp=" + item.innerText; 
...
insertBtn?.addEventListener("click", () => {
  ...
  // 현재 주소 : /board/{boardCode}       select == board 
  // 요청 주소 : /editBoard/{boardCode}/insert      update,del,insert == editBoard
