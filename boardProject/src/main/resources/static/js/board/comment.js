...
const selectCommentList = () => {
  ...
  .then(html => {
    ...
    /* [주의 사항] */
    // innerHTML 로 새로 만들어진 요소에는 
    // 이벤트 리스너가 추가되어있지 않기 때문에
...
/** 
 * 댓글 등록 함수 (AJAX)
 * @param parentCommentNo : 부모 댓글 번호 (없으면 undefined)
 */
...
*/
  ...
  .then(commentNo => {
    ...
    alert("댓글이 등록 되었습니다");
    commentContent.value = ""; // textarea 에 남아있는 댓글 내용 삭제
    selectCommentList(); // 댓글 목록 비동기 조회 후 출력


    // 댓글을 작성한 경우
    // -> {닉네임} 님이 {게시글 제목} 게시글에 댓글을 작성했습니다
    // -> 알림 클릭 시 이동하는 url 에 ?cn= 댓글 번호 추가
    if (parentCommentNo === undefined) {          // boardDetail.html 에서 가져옴
...
const showUpdateComment = (btn) => {
  ...
  // (?) : 그냥 댓글 목록 새로 불러오면 안되는 건지?
  // selectCommentList()
  // 답글 여러 개일때 새 화면 불러오기 어려움
  ...
  */
  ...
  // 3. 기존 댓글에 작성된 내용만 얻오오기, 2번째 자식 요소
  ...
  cancelBtn.addEventListener("click", () => {
    ...
    // 현재 댓글행 다음 위치(형제)에 백업한 원본 댓글 추가(append(자식) 와 유사)
    commentRow.after(beforeCommentRow);

    // 수정화면으로 변환된 행 삭제
    commentRow.remove();

    /* 원상 복구된 댓글 버튼에 이벤트 추가하기 */
    const childCommentBtn     // 답글 버튼
...
/**
 * 화면에 존재하는 모든 댓글 수정 버튼에 이벤트 리스너 추가하는 함수
 */
