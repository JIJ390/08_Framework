...
const connectSse = () => {
  ...
  eventSource.addEventListener("message", e => {
    ...
    notificationBtn.classList.add("fa-solid");       // 색 채우기
    notificationBtn.classList.remove("fa-regular");  // 색 비우기
...
const notReadCheck = () => {
  ...
  .then(count => {
    ...
    // 읽지 않은 게시글의 수가 0 보다 크다면
    // == 읽지 않은 알림이 존재한다! == 노란색으로 불들어로게 하기
    if(count > 0) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  notificationBtn?.addEventListener("click", () => {
    ...
    // notification-show 가지고 있으면 목록이 보이는 상태
    if (notificationList.classList.contains("notification-show")) {
      // 안보이게 하기
  ...
  */
  ...
  if(cn != null) {
    // cn 이 존재하는 경우
    const targetId = "c" +cn; // c100, c23232 형태로 변환
    ...
    // 창을 스크롤 
    window.scrollTo({
      top : scrollPosition - 200 , // 스크롤할 길이, px 단위
