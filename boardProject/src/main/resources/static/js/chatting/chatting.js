...
- ws:// 프로토콜 요청 (http 가 아님!!!!)
 -> HTTP 에서 사용하는 request, session 을 사용할 수 없음

 --> SessionHandShakeInterceptor 가
 HTTP 의 Session 을 가로채서 Websocket 에서 사용 가능한 형태로 변환
...
const resultArea = document.querySelector("#resultArea"); // 검색창
...
targetInput.addEventListener("input", () => {
  ...
  // 입력된 값이 있을 경우  / get 방식 쿼리 스트링으로 전달
...
const chattingEnter = (e) => {
  ...
  .then(chattingNo => {
    ...
    setTimeout(() => {
      ...
      // 2) li 태그의 chat-no 값과 입장하려는
      //    방 번호가 같은 경우
      //    == 입장하려는 방이 채팅방 목록에 존재하는 경우
      for (let item of itemList) {
        if (item.getAttribute("chat-no")) {
          item.focus();
          item.click();
