...
public class MainController {
	...
	public String mainPage(
		...
		// - 주소 + 쿼리 스트링 조합을 쉽게 할 수 있는 기능 제공
		// - 자동을 'URL 인코딩' 처리하여 안전하게 요청 가능
		// * 'URL 인코딩' : 자동으로 URL 사용 가능 형태로 변환
		...
		// decodingServiceKey 를 그대로 넣으면 추가 구문 존재 시 중복 인코딩 발생
		// {} 내부는 인코딩 하지 않음!!
		...
		// fetch("주소", {method "POST", headers, body});
		// 요청 헤더!!
		HttpHeaders headers = new HttpHeaders();
		headers.set("Content-Type", "application/json");
		
		// HTTP 요청 헤더와 바디를 묶어주는 객체 (fetch API 형태를 보면서 이해)
		...
		try {
			ResponseEntity<String> response
				= restTemplate.exchange(
						uriString,			// 요청 주소 + 쿼리스트링(여기서 한 번 더 URL 인코딩 수행) 
			...
			// Jackson 라이브러리
			// -> Java 에서 JSON 을 다룰 수 있게 해주는 라이브러리
			
			// Jackson data-bind 라이브러리
			// -> JSON 데이터를 Java 객체(Map,DTO) 로 변환할 수 있는 라이브러리
			
			// spring-starter-web dependency 에 Jackson data-bind 포함되어 있음
			
			// JSON 을 쉽게 다루거나 변환할 수 있는 객체
