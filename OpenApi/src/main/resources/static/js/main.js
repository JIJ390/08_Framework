...
const getAirQuality = async (cityName) => {
  ...
  /*
    ...
    - await 키워드가 붙은 함수가 종료될 때까지 코드 해석을 일시 중지
      (동기식)

  */

  // /getServuceKey 요청에 대한 응답이 돌아올 때까지 대기(await)
  // 응답이 돌아오면 response1 변수에 
  // 응답 결과(첫번째 then의 response) 가 저장됨
  const response1 = await fetch('/getServiceKey', {method : "POST"});
    
  // 응답 데이터를 text 형태로 파싱될 때까지 대기(await)
  // -> serviceKey 에 서버에서 조회한 값이 저장
...
// Java 영역 select 변경 후 조회 클릭 시에 대한 동작
